...
class MultiProcessorPoolHandler:
    ...
    def __init__(self, parallel_size):
        ...
        self.running_jobs = {}
        self.start_thread = threading.Thread(target=MultiProcessorPoolHandler.start_thread_fun, name='start_thread', args=(self,))
        self.clean_thread = threading.Thread(target=MultiProcessorPoolHandler.clean_thread_fun, name='clean_thread', args=(self,))
    ...
    def exec_strategy(self, strategy_name, args):
        if not self.waiting_jobs.full():
            key = uuid.uuid1()
            self.waiting_jobs.put((key, self.strategies[strategy_name], args,))
            return key
        else:
            return None
...
class StrategyBuilder:
    def __init__(self, strategy_name, fun, wrapper, *wrapper_args):
    ...
    def get_fun_(self):
        if self.wrapper:
            return self.wrapper(self.fun, self.wrapper_args)
        else:
            return self.fun
...
class LogWrap:
    ...
    LogFileLocation = enum('REAL_STR', 'PARAM_INDEX', 'COMPLEX')

    def __init__(self, func, wrapper_args):
        self.func = func
        self.wrapper_args = wrapper_args
        # self.wrapper_kwargs = wrapper_kwargs
        # self.log_file_structure = {"type":None, "path":None, "filename":None}
        self.path = None
        self.filename = None

    def __call__(self, *args, **kwargs):
        if isinstance(self.wrapper_args, tuple) and len(self.wrapper_args) == 2:
            if isinstance(self.wrapper_args[0], int) and isinstance(self.wrapper_args[0], int):
                self.path = args[self.wrapper_args[0]]
                self.filename = args[self.wrapper_args[1]]
            elif isinstance(self.wrapper_args[0], str) and isinstance(self.wrapper_args[0], str):
                self.path = self.wrapper_args[0]
                self.filename = self.wrapper_args[1]
        else:
            self.path = "."
        ...
        with open(self.path[:-1] if self.path.endswith('/') else self.path + '/' + self.filename, 'w+') as file_:
            sys.stdout = file_
            sys.stderr = file_
...
class TrainImage(image_train_service_pb2_grpc.TrainServiceServicer):
    ...
    def DoTrain(self, request, context):
        ...
        # train_no = request.text
        # train_no = u'T201706221606037305'　invoke_image_train,
        # u'/data/jg/image/train/tmp/logs/' + train_no, '/stdout.txt'
        train_handler.exec_strategy('image_train', (train_no, train_step, 2, u'/data/jg/image/train/tmp/logs/' + train_no, u'/stdout.txt'))
        return image_train_service_pb2.TrainResponse(result_no=1)
...
def server():
    ...
    train_handler = MultiProcessorPoolHandler(1)
